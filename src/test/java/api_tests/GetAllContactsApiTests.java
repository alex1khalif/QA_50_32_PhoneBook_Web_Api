...
public class GetAllContactsApiTests implements BaseApi, ILogin {
    ...
    public void getAllUserContactsPositiveApiTest(){
        ...
        Request request = new Request.Builder().url(BASE_URL + GET_ALL_CONTACTS_URL)
                .addHeader(AUTH, token.getToken()).get().build();
    ...
    public void getAllUserContactsNegativeWrongTokenApiTest(){
        ...
        try(Response response = OK_HTTP_CLIENT.newCall(request).execute()) {
            ...
            softAssert.assertTrue(errorMessageDto.getMessage()
                    .contains("strings must contain exactly 2 period characters."), "validate message");
