...
public class UpdateContactApiTests implements BaseApi, ILogin {
    ...
    public void updateContactPositiveApiTest(){
        ...
        RequestBody requestBody = RequestBody.create(GSON.toJson(contact), JSON);
        Request request = new Request.Builder().url(BASE_URL + EDIT_CONTACT_URL)
                .addHeader(AUTH, token.getToken()).put(requestBody).build();
        try(Response response = OK_HTTP_CLIENT.newCall(request).execute()){
            softAssert.assertEquals(response.code(), 200, "validate status code");
                ResponseMessageDto responseMessageDto =
                        GSON.fromJson(response.body().string(), ResponseMessageDto.class);
            //System.out.println(responseMessageDto);
                softAssert.assertTrue(responseMessageDto.getMessage().contains(""), "validate message");
